...
#ifndef ZK_DTYPES__SRC_INTN_NUMPY_H_
#define ZK_DTYPES__SRC_INTN_NUMPY_H_
...
constexpr char kIntNOutOfRange[] =
    "out of range value cannot be converted to intN";
...
bool CastToIntN(PyObject* arg, T* output) {
  ...
  if (PyArray_IsScalar(arg, Integer)) {
    ...
    if (!(std::numeric_limits<T>::min() <= v &&
      ...
      PyErr_SetString(PyExc_OverflowError, kIntNOutOfRange);
      return false;
    }
    *output = T(v);
    return true;
...
PyObject* PyIntN_tp_new(PyTypeObject* type, PyObject* args, PyObject* kwds) {
  ...
  if (PyIntN_Check<T>(arg)) {
    Py_INCREF(arg);
    return arg;
  } else if (CastToIntN(arg, &value)) {
    return PyIntN_FromValue(value).release();
...
PyObject* PyIntN_nb_int(PyObject* self) {
  ...
  return PyLong_FromLong(static_cast<long>(x));
}

template <typename T>
PyObject* PyIntN_nb_negative(PyObject* self) {
  T x = PyIntN_Value_Unchecked<T>(self);
  return PyIntN_FromValue(-x).release();
}
...
PyObject* PyIntN_nb_add(PyObject* a, PyObject* b) {
  T x, y;
  if (PyIntN_Value(a, &x) && PyIntN_Value(b, &y)) {
    return PyIntN_FromValue(x + y).release();
  }
...
PyObject* PyIntN_nb_remainder(PyObject* a, PyObject* b) {
  ...
  if (PyIntN_Value(a, &x) && PyIntN_Value(b, &y)) {
    ...
    return PyIntN_FromValue(v).release();
  }
...
int NPyIntN_SetItem(PyObject* item, void* data, void* arr) {
  T x;
  if (PyIntN_Check<T>(item)) {
    x = PyIntN_Value_Unchecked<T>(item);
  } else if (!CastToIntN(item, &x)) {
    return -1;
  }
...
void NPyIntN_IntegerCast(void* from_void, void* to_void, npy_intp n,
  ...
  for (npy_intp i = 0; i < n; ++i) {
    to[i] = static_cast<typename TypeDescriptor<To>::T>(
        static_cast<To>(NPyIntN_CastToInt(from[i])));
  }
}

// Registers a cast between 'T' and type 'OtherT'. 'numpy_type' is the NumPy
// type corresponding to 'OtherT'.
...
bool RegisterIntNCasts() {
  ...
  if (!RegisterCustomIntCast<T, unsigned short>(NPY_USHORT)) {
    return false;
  }
  if (!RegisterCustomIntCast<T, unsigned int>(NPY_UINT)) {
    return false;
  }
  if (!RegisterCustomIntCast<T, unsigned long>(NPY_ULONG)) {
    return false;
  }
  if (!RegisterCustomIntCast<T, unsigned long long>(NPY_ULONGLONG)) {
    return false;
  }
  if (!RegisterCustomIntCast<T, signed char>(NPY_BYTE)) {
    return false;
  }
  if (!RegisterCustomIntCast<T, short>(NPY_SHORT)) {
    return false;
  }
  if (!RegisterCustomIntCast<T, int>(NPY_INT)) {
    return false;
  }
  if (!RegisterCustomIntCast<T, long>(NPY_LONG)) {
    return false;
  }
  if (!RegisterCustomIntCast<T, long long>(NPY_LONGLONG)) {
    return false;
  }
...
#endif  // ZK_DTYPES__SRC_INTN_NUMPY_H_
