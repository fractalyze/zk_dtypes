...
FIELD_TYPES = [
  babybear, babybear_std,
  goldilocks, goldilocks_std,
  koalabear, koalabear_std,
  mersenne31, mersenne31_std,
  bn254_sf, bn254_sf_std,
]
...
class ScalarTest(parameterized.TestCase):
  ...
  def testRoundTripToPythonScalar(self, scalar_type, python_scalar):
    for v in VALUES[scalar_type]:
      self.assertEqual(python_scalar(normalize(scalar_type, v)), python_scalar(scalar_type(v)))
      self.assertEqual(
  ...
  def testRoundTripNumpyTypes(self, scalar_type):
    for dtype in [np.uint64]:
      ...
      np.testing.assert_equal(
          ...
          np.array([normalize(scalar_type, v) for v in VALUES[scalar_type]], dtype),
      )

  @parameterized.product(scalar_type=FIELD_TYPES)
  def testStr(self, scalar_type):
    for value in VALUES[scalar_type]:
      self.assertEqual(str(normalize(scalar_type, value)), str(scalar_type(value)))
  ...
  def testComparison(self, scalar_type, op):
    for v in VALUES[scalar_type]:
      for w in VALUES[scalar_type]:
        ...
        self.assertEqual(op(normalize(scalar_type, v), normalize(scalar_type, w)), result)
    ...
    for v in VALUES[scalar_type]:
      for w in VALUES[scalar_type]:
        if v == 0 and w < 0:
          ...
          out = scalar_type(v) ** w
          self.assertIsInstance(out, scalar_type)
          self.assertEqual(scalar_type(pow(v, w, MODULI[scalar_type])), out, msg=(v, w))
...
class ArrayTest(parameterized.TestCase):
  ...
  def testUnaryPredicateUfunc(self, scalar_type, ufunc):
    ...
    x = np.array([normalize(scalar_type, x) for x in VALUES[scalar_type]], dtype=np.uint64)
