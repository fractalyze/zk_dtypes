...
int2 = zk_dtypes.int2
int4 = zk_dtypes.int4
uint2 = zk_dtypes.uint2
uint4 = zk_dtypes.uint4
...
class ArrayTest(parameterized.TestCase):
  ...
  def testCasts(self, scalar_type, dtype):
    if (
        (scalar_type == int2 and dtype == np.uint64)
        or (scalar_type == int2 and dtype == np.ulonglong)
        or (scalar_type == int4 and dtype == np.uint64)
        or (scalar_type == int4 and dtype == np.ulonglong)
    ):
      self.skipTest(
          "Casting int2 and int4 to np.uint64 or np.ulonglong "
          "requires supporting casting to np.longdouble, "
          "which is not implemented in zk_dtypes."
      )
