 ...
 public:
  ...
  Derived operator*(const Derived& other) const {
    ...
    if (!self.HasSimpleNonResidue()) {
      ...
      ExtensionFieldMulAlgorithm algorithm = self.GetMulAlgorithm();
  ...
  Derived Square() const {
    ...
    if (!self.HasSimpleNonResidue()) {
      ...
      if (algorithm == ExtensionFieldMulAlgorithm::kCustom) {
        ...
        // Default Karatsuba:
        // - square: 4, mul: 6, mul by non-residue: 3, add: 12, sub: 0,
        //   double: 6
        //
        // Conclusion:
        ...
        const std::array<BaseField, 4>& x = self.ToCoeffs();
        ...
        return self.FromCoeffs({y0, y1, y2, y3});
  ...
  Derived Inverse() const {
    ...
    if (!self.HasSimpleNonResidue()) {
      ...
      // Optimized algorithm for X⁴ = ξ (Tower Extension)
      // [Comparison]
      ...
      const std::array<BaseField, 4>& x = self.ToCoeffs();
      BaseField xi = self.NonResidue();  // ξ
      ...
      // 3) Compute the norm N = D₀² − ξ·D₁² ∈ Fp and its inverse N⁻¹ in the
      // base field. Inverse() returns Zero() if not invertible.
