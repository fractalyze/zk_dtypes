...
#ifndef ZK_DTYPES_INCLUDE_BYINVERTER_H_
#define ZK_DTYPES_INCLUDE_BYINVERTER_H_
...
// Big signed (B * L)-bit integer type, whose variables store numbers in the
// two's complement code as arrays of B-bit chunks, where B < 64 and L > 0.
// The ordering of the chunks in these arrays is little-endian. The arithmetic
// operations for this type are wrapping ones
template <size_t B, size_t L>
class CInt {
 public:
  // Mask where only the B lowest bits are set to 1, all other bits are 0
  constexpr static uint64_t MASK = ~uint64_t{0} >> (64 - B);

  // Returns the number equal to the argument. The binary representation of the
  // absolute value of the argument must have no more than B * L - 1 bits to
  // ensure that the result and the argument represent the same number
  constexpr static CInt Load(const int64_t input) {
    CInt result;
    uint64_t raw = input;
    Convert<64, B>(&raw, 1, result.data, L);
    if ((B * L > 64) && (input < 0)) {
      // For the two's complement code and n > m the n-bit representation of a
      // negative number can be expressed as its m-bit representation preceded
      // by m - n bits equal to 1
  ...
  // Returns the number, the absolute value of which is specified by the first
  // argument. A non-zero return number is negative iff "sign" is "true".
  // The binary representation of the absolute value of the input number
  ...
  // Returns the current number. The binary representation of its absolute
  // value must have no more than 63 bits to ensure that the result represents
  // the current number
  ...
  // Returns the sign of the current number and stores its absolute value into
  // the variable, which is specified by the argument. The binary
  // representation of the absolute value of the current number must contain no
  ...
  // Returns the result of applying B-bit right arithmetical shift to the
  // current number
  ...
  constexpr CInt operator-(const CInt& other) const {
    ...
    // bitwise inversion and addition is done according to the rules of the
    // code. The algorithm below uses this formula and is the modified addition
    // algorithm, where the carry flag is initialized with 1 and the chunks of
    // the second argument are bitwise inverted
    CInt result;
    uint64_t carry = 1;
    for (size_t i = 0; i < L; i++) {
  ...
  constexpr CInt operator-() const {
    // For the two's complement code the additive negation is the result of
    // adding 1 to the bitwise inverted argument's representation
    CInt result;
    uint64_t carry = 1;
    for (size_t i = 0; i < L; i++) {
  ...
  constexpr CInt operator*(const CInt& other) const {
    ...
    for (size_t i = 0; i < L; i++) {
      ...
      for (size_t k = i; k < L; k++) {
        absl::uint128 sum =
            absl::uint128{data[i]} * other.data[k - i] + result.data[k] + carry;
  ...
  constexpr CInt operator*(int64_t other) const {
    ...
    for (size_t i = 0; i < L; i++) {
      absl::uint128 sum = absl::uint128{data[i] ^ mask} * other + carry;
 ...
 private:
  ...
  // Stores at the address "output" the array of O-bit chunks, which represents
  // the big unsigned integer equal modulo 2ᴼ * "ᵒˢᶦᶻᵉ" to the input big
...
// adjusting parameter A to compute the adjusted multiplicative inverses of
// positive integers, i.e. for computing (1 / x) * A (mod M) for a positive
// integer x.
//
...
class BYInverter {
 public:
  ...
  // Returns "true" and stores into the variable, which is specified by the
  ...
  bool Invert(const BigInt<L>& input, BigInt<L>& output) const {
    ...
    // At this point the absolute value of "f" equals the greatest common
    // divisor of the integer to be inverted and the modulus the inverter was
    // created for. Thus, if "f" is neither 1 nor -1, then the sought inverse
    // does not exist
 ...
 private:
  // The big signed integer type used by the inverter. The absolute values of
  // big signed results of the intermediate computations have no more than
  // 64 * L + 63 bits in their binary representation
  ...
  // Stores the Bernstein-Yang transition matrix multiplied by 2⁶² and the new
  // value of the delta variable for the 62 basic steps of the Bernstein-Yang
  // method, which are to be performed sequentially for specified initial values
  ...
  static void Jump(uint64_t f, uint64_t g, int64_t& delta, int64_t t[2][2]) {
    ...
    while (true) {
      int64_t zeros = absl::countr_zero(g);
  ...
  // Stores the updated values of the variables f and g for specified initial
  // ones and Bernstein-Yang transition matrix multiplied by 2⁶². In the vector
  ...
  // Stores the updated values of the variables d and e for specified initial
  // ones and Bernstein-Yang transition matrix multiplied by 2⁶². The new value
  // of the vector "(d, e)'" is congruent modulo M to "matrix * (d, e)' / 2⁶²
  ...
  // multiplicative inverse modulo a power of two. For better understanding the
  // implementation, the following paper is recommended:
...
#endif  // ZK_DTYPES_INCLUDE_BYINVERTER_H_
